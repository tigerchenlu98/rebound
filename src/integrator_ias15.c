...
static int reb_integrator_ias15_step_internal(struct reb_simulation* r) {
    ...
    while (1) {
        ...
        for (int n = 1; n < 8; n++) { // Loop over interval using Gauss-Radau spacings
            ...
            for (int i = 0; i < N; i++) { // Predict positions at interval n using b values
                ...
                double xk0      = -csx[k0] + ((((((((b.p6[k0] * 7. * h[n] / 9. + b.p5[k0]) * 3. * h[n] / 4. + b.p4[k0]) * 5. * h[n] / 7. + b.p3[k0]) * 2. * h[n] / 3. + b.p2[k0]) * 3. * h[n] / 5. + b.p1[k0]) * h[n] / 2. + b.p0[k0]) * h[n] / 3. + a0[k0]) * r->dt * h[n] / 2. + v0[k0]) * r->dt * h[n];
                double xk1      = -csx[k1] + ((((((((b.p6[k1] * 7. * h[n] / 9. + b.p5[k1]) * 3. * h[n] / 4. + b.p4[k1]) * 5. * h[n] / 7. + b.p3[k1]) * 2. * h[n] / 3. + b.p2[k1]) * 3. * h[n] / 5. + b.p1[k1]) * h[n] / 2. + b.p0[k1]) * h[n] / 3. + a0[k1]) * r->dt * h[n] / 2. + v0[k1]) * r->dt * h[n];
                double xk2      = -csx[k2] + ((((((((b.p6[k2] * 7. * h[n] / 9. + b.p5[k2]) * 3. * h[n] / 4. + b.p4[k2]) * 5. * h[n] / 7. + b.p3[k2]) * 2. * h[n] / 3. + b.p2[k2]) * 3. * h[n] / 5. + b.p1[k2]) * h[n] / 2. + b.p0[k2]) * h[n] / 3. + a0[k2]) * r->dt * h[n] / 2. + v0[k2]) * r->dt * h[n];
            ...
            if (r->calculate_megno || (r->additional_forces && r->force_is_velocity_dependent)) {
                for (int i = 0; i < N; i++) { // Predict velocities at interval n using b values
                    ...
                    double vk0       = -csv[k0] + (((((((b.p6[k0] * 7. * h[n] / 8. + b.p5[k0]) * 6. * h[n] / 7. + b.p4[k0]) * 5. * h[n] / 6. + b.p3[k0]) * 4. * h[n] / 5. + b.p2[k0]) * 3. * h[n] / 4. + b.p1[k0]) * 2. * h[n] / 3. + b.p0[k0]) * h[n] / 2. + a0[k0]) * r->dt * h[n];
                    double vk1       = -csv[k1] + (((((((b.p6[k1] * 7. * h[n] / 8. + b.p5[k1]) * 6. * h[n] / 7. + b.p4[k1]) * 5. * h[n] / 6. + b.p3[k1]) * 4. * h[n] / 5. + b.p2[k1]) * 3. * h[n] / 4. + b.p1[k1]) * 2. * h[n] / 3. + b.p0[k1]) * h[n] / 2. + a0[k1]) * r->dt * h[n];
                    double vk2       = -csv[k2] + (((((((b.p6[k2] * 7. * h[n] / 8. + b.p5[k2]) * 6. * h[n] / 7. + b.p4[k2]) * 5. * h[n] / 6. + b.p3[k2]) * 4. * h[n] / 5. + b.p2[k2]) * 3. * h[n] / 4. + b.p1[k2]) * 2. * h[n] / 3. + b.p0[k2]) * h[n] / 2. + a0[k2]) * r->dt * h[n];
    ...
    if (config->epsilon > 0) {
        ...
        if (fabs(dt_new) < config->min_dt)
            dt_new = copysign(config->min_dt, dt_new);
...
void* reb_integrator_ias15_new(struct reb_integrator* integrator, struct reb_simulation* r) {
...
void reb_integrator_ias15_register(struct reb_simulation* r) {
    ...
    integrator->new         = reb_integrator_ias15_new;
