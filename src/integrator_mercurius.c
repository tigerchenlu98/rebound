...
static void reb_mercurius_encounter_predict(struct reb_simulation* const r) {
    ...
    for (int i = 0; i < N_active; i++) {
        for (int j = i + 1; j < N; j++) {
            ...
            if (tmin1 > 0. && tmin1 < 1.) {
                const double rmin1 = (1. - tmin1) * (1. - tmin1) * (1. + 2. * tmin1) * ro + tmin1 * tmin1 * (3. - 2. * tmin1) * rn + tmin1 * (1. - tmin1) * (1. - tmin1) * dt * drodt - tmin1 * tmin1 * (1. - tmin1) * dt * drndt;
                rmin               = MIN(MAX(rmin1, 0.), rmin);
            }
            if (tmin2 > 0. && tmin2 < 1.) {
                const double rmin2 = (1. - tmin2) * (1. - tmin2) * (1. + 2. * tmin2) * ro + tmin2 * tmin2 * (3. - 2. * tmin2) * rn + tmin2 * (1. - tmin2) * (1. - tmin2) * dt * drodt - tmin2 * tmin2 * (1. - tmin2) * dt * drndt;
...
void* reb_integrator_mercurius_new(struct reb_integrator* integrator, struct reb_simulation* r) {
...
void reb_integrator_mercurius_register(struct reb_simulation* r) {
    ...
    integrator->new             = reb_integrator_mercurius_new;
