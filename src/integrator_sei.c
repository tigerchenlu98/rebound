...
size_t reb_integrator_sei_load(struct reb_integrator* integrator,
                               struct reb_simulation* r,
                               struct reb_input_stream* stream,
                               struct reb_binary_field field) {
    struct reb_integrator_sei_config* config =
        (struct reb_integrator_sei_config*)integrator->config;
...
void reb_integrator_sei_save(struct reb_integrator* integrator,
                             struct reb_simulation* r,
...
void* reb_integrator_sei_new(struct reb_integrator* integrator,
                             struct reb_simulation* r) {
    printf("alloc\n");
    struct reb_integrator_sei_config* config = calloc(
        1,
        sizeof(struct reb_integrator_sei_config)); // sets all variables to zero
    return config;
}
void reb_integrator_sei_free(struct reb_integrator* integrator,
                             struct reb_simulation* r) {
    struct reb_integrator_sei_config* config =
        (struct reb_integrator_sei_config*)integrator->config;
    free(config);
 ...
 */
static void operator_H012(double dt, const double Omega, const double Omega_z,
                          const struct reb_integrator_sei_config config,
    ...
    const double aO =
        2. * p->vy + 4. * p->x * Omega; // Center of epicyclic motion
 ...
 * @details It is only exact if the forces are velocity independet (i.e.
 * gravity). If the forces are velocity dependent, it breaks the symmetry of the
 * scheme, making it firsr-order and non-symplectic. As long as these forces are
 * small, this should not be visible. However, it is worth keeping in mind.
...
void reb_integrator_sei_step(struct reb_integrator* integrator,
...
void reb_integrator_sei_register(struct reb_simulation* r) {
    struct reb_integrator* integrator =
        reb_simulation_register_integrator(r, "sei", 2);
