...
int reb_binary_diff(struct reb_input_stream* istream1, struct reb_input_stream* istream2, struct reb_output_stream* ostream, int output_option) {
    ...
    while (1) {
        ...
        if (pos1 + field1.size > istream1->size)
            printf("Corrupt binary file istream1->mem_stream.\n");
        if (pos2 + field2.size > istream2->size)
            printf("Corrupt binary file istream2->mem_stream.\n");
        int fields_differ = 0;
