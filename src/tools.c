...
void reb_tools_megno_init(struct reb_simulation* const r) {
    ...
    for (; i < imax; i++) {
        ...
        double deltad   = 1. / sqrt(
                                 particles[i].x * particles[i].x + particles[i].y * particles[i].y + particles[i].z * particles[i].z + particles[i].vx * particles[i].vx + particles[i].vy * particles[i].vy + particles[i].vz * particles[i].vz); // rescale
...
void reb_tools_megno_update(struct reb_simulation* r, double dY) {
    ...
    r->megno_cov_Yt += ((double)r->megno_n - 1.) / (double)r->megno_n * (r->t - r->megno_mean_t) * (reb_tools_calculate_megno(r) - r->megno_mean_Y);
